var TB = {
  ...
  getOppositDirection: function (dir) {
    if (dir === C.UP) return C.DOWN
    if (dir === C.DOWN) return C.UP
    if (dir === C.LEFT) return C.RIGHT
    if (dir === C.RIGHT) return C.LEFT
  },

  destroyWrap: function (obj) {
    return function () {
...
setTimeout(() => {
  ...
  Test.test('getOppositDirection 1', TB.getOppositDirection(C.UP) === C.DOWN)
  Test.test('getOppositDirection 2', TB.getOppositDirection(C.DOWN) === C.UP)
  Test.test('getOppositDirection 3', TB.getOppositDirection(C.LEFT) === C.RIGHT)
  Test.test('getOppositDirection 4', TB.getOppositDirection(C.RIGHT) === C.LEFT)

  let destroyWrapHelper = {tom: 7}
  TB.destroyWrap(destroyWrapHelper)()
  Test.test('destroyWrap', destroyWrapHelper.pendingDestroy === true)
